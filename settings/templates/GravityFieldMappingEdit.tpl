            ...
            <div class="contents" id="detailView">
                <table class="table table-bordered" width="100%" id="convertLeadMapping">
                    <tbody>
                        ...
                        {foreach key=MAPPING_ID item=MAPPING_ARRAY from=$MODULE_MODEL->getMapping({$FORMID})  name="mappingLoop"}
                            <tr class="listViewEntries" sequence-number="{$smarty.foreach.mappingLoop.iteration}">
                                ...
                                <td width="13%">          
                                    <select class="contactFields select2" style="width:180px" name="mapping[{$smarty.foreach.mappingLoop.iteration}][gravity]">
                                        
                                         {foreach key=ID item=FIELDS_INFO from=$GRAVITY_MODULE_MODEL}        
                                                    <option {if $ID eq $MAPPING_ARRAY['Gravity']['id']} selected {/if} value="{$ID}">
                                                        {$FIELDS_INFO['0']}
                                                    </option>  
                                        {/foreach}
                                    </select>    
